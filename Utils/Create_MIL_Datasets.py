...
class Create_MIL_Datasets():
    def __init__(self, h_step = 2, w_step = 2, read_path = None, save_path = None, resize_shape = (96, 96),
                 resize_stat = False, del_white_map = False, del_white_thod = 0.9, add_num = 0):
        self.h_step = h_step
        self.w_step = w_step
        ...
        for img_name in img_name_list:
            ...
            for i in range(self.h_step):
                for j in range(self.w_step):
                    ...
                    if self.del_white_map == True:
                        if np.mean(color.rgb2gray(inter_img)) < self.del_white_thod:
                            io.imsave(self.save_path + '//' + new_file_name + '//' +
                                      str(((i * self.w_step) + j) + self.add_num) + '.jpg', inter_img.astype('uint8'))
                    else:
                        if np.mean(color.rgb2gray(inter_img)) > self.del_white_thod:
                            inter_img = np.zeros_like(inter_img)
                        io.imsave(self.save_path + '//' + new_file_name + '//' +
                    ...
                    print(img_name + '  ' + str(((i * self.w_step) + j) + self.add_num))
...
if __name__ == '__main__':
    read_path = r'E:\DHM_MIL\Datasets\Larynx\Org_IMG\Test\III'
    save_path = r'E:\DHM_MIL\Datasets\Larynx\Larynx_16\Test\III'
    create_mil_obj = Create_MIL_Datasets(read_path = read_path, save_path = save_path, add_num=69,
                                         w_step = 4, h_step = 4, del_white_map=False, del_white_thod=0.9,
